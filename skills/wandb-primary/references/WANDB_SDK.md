...
summaries = []
for rid in run_ids:
    r = api.run(f"{path}/{rid}")
    summaries.append({
        "run": r.name,
        "final_loss": r.summary_metrics.get("loss"),
        "final_val_loss": r.summary_metrics.get("val_loss"),
        "min_loss": None,  # fill from history
    })

# For detailed comparison, load histories
for i, rid in enumerate(run_ids):
    r = api.run(f"{path}/{rid}")
    df = pd.DataFrame(list(r.scan_history(keys=["loss"])))
    summaries[i]["min_loss"] = df["loss"].min()

compare_df = pd.DataFrame(summaries).sort_values("final_loss")
...
# Report smoothed final performance
...
# Spikes: points > 3σ from rolling mean
threshold = 3.0
spikes = loss[(loss - rolling_mean).abs() > threshold * rolling_std]
print(f"Spikes (>{threshold}σ): {len(spikes)}")
