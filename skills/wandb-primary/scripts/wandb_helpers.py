"""Standalone helpers for working with Weave and W&B data.

These don't depend on weave_tools — they work directly with the weave SDK
objects returned by client.get_call() and client.get_calls().

Usage (in sandbox):
    import sys
    sys.path.insert(0, "skills/wandb-primary/scripts")
    from wandb_helpers import unwrap, get_token_usage, eval_results_to_dicts
"""
...
def unwrap(obj: Any) -> Any:
    ...
    Handles WeaveDict, WeaveObject, ObjectRecord, ObjectRef, and nested
    combinations thereof. Safe to call on already-plain objects.
