...
Optimized for large projects (10K+ runs, 1K+ metrics per run).

Key optimizations vs the base wandb-primary skill:
- Uses wandb.Api(timeout=60) to prevent timeouts on large projects
- runs_to_dataframe: skips config iteration by default, uses per_page tuning
- diagnose_run: uses beta_scan_history (parquet) for large histories,
  accepts configurable metric keys
- All history methods ALWAYS require explicit keys to avoid 502s on
  runs with thousands of metrics
- Server-side filters preferred over client-side iteration
