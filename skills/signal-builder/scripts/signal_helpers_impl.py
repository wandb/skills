...
def explore_ops(
    ...
    Queries root-level calls, groups by op name, and for each op samples
    a few calls to infer the field structure. This helps you understand
    what data is available before writing a signal prompt.
    ...
    client = weave.weave_client()

    # Fetch a batch of recent root calls to discover ops.
...
def sample_traces(
    ...
    Initializes Weave for the given entity/project, queries calls using a
    root-traces-only filter, and returns a list of unwrapped trace dicts.
    ...
    filter_kwargs: dict[str, Any] = {"trace_roots_only": True}
    if op_name:
    ...
    if status == "success":
        query = SUCCESSFUL_ROOT_TRACES_QUERY
    elif status == "error":
...
def create_signal(
    ...
    monitor_val = {
        ...
        "description": description or "",
        "scorers": [scorer_ref],
        "query": query or SUCCESSFUL_ROOT_TRACES_QUERY,
        "prompt_header": header,
        "prompt_footer": footer,
        "op_names": _normalize_op_names(op_names),
